...
// Garante execução da criação da tabela
initDB();

/*
 * Criar Ofício
 * Reinicia a numeração a cada ano
*/
export async function criarOficio(data: {
  ...
  // Busca o último número utilizado no ano
  const result = await db.execute({
    sql: "SELECT numero FROM oficios WHERE ano = ? ORDER BY numero DESC LIMIT 1",
    args: [data.ano],
  });

  const lastNumber = result.rows.length > 0 ? Number(result.rows[0].numero) : 0;
...
export async function listarOficios() {
  const result = await db.execute("SELECT * FROM oficios ORDER BY ano DESC, numero DESC");
  return result.rows;
}
